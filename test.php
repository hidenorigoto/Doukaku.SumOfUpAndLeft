...
function test($question, $expected) {
    echo 'Q:' . $question . '    ';
    echo 'A:';

    $sheet = new \Doukaku\SumOfUpAndLeft\Model\Sheet($question);

    $sheet->cells[0][0]->setValue(1);
    $sheet->evaluate();

    $result = sprintf('%02d', $sheet->getResult());

    echo $result . ' ' . (($result == $expected) ? 'OK' : 'NG');
    echo PHP_EOL;
}
